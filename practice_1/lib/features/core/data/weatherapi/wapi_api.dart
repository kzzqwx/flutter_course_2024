...
class WApi {
  ...
  Future<WAPIWeather> getWeather(String city) async {
    var response = await http.get(
        Uri.parse('$url/current.json?key=$apiKey&q=$city'));
    var rjson = jsonDecode(response.body);
    return WAPIWeather(
        rjson['current']['temp_c'], rjson['current']['condition']['text']);
  }

  Future<WAPIWeather> getWeatherByCoords(double lat, double longt) async {
    var res = await http.get(
        Uri.parse('$url/current.json?key=$apiKey&q=$lat,$longt'));
    var rjson = jsonDecode(res.body);
    return WAPIWeather(
        rjson['current']['temp_c'], rjson['current']['condition']['text']);
  }
}
